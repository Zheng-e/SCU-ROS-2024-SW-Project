...
def callback(data):
    ...
    if data.command == "return":
        ...
        if initial_pose:
            # 杀死原来的乌龟
            try:
                kill_turtle('scuGroupidFollowerB')
...
def followerA():
    ...
    rospy.init_node('scuGroupidFollowerA', anonymous=True)
    pub = rospy.Publisher('/scuGroupidFollowerA/cmd_vel', Twist, queue_size=10)
    in_position_pub = rospy.Publisher('/scuGroupidFollowerA/in_position', Empty, queue_size=10)
    rospy.Subscriber('/scuGroupidLeader/info', GroupidLeaderMessage, callback)
    rospy.Subscriber('/scuGroupidFollowerA/pose', Pose, update_follower_pose)
